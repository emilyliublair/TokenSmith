...
run-index-partial:
	...
	conda run --no-capture-output -n tokensmith2 python -m src.main index --partial --chapters $(CHAPTERS) $(ARGS)

run-add-chapters-partial:
	@echo "Adding chapters $(CHAPTERS) to partial index with ARGS: $(ARGS)"
	conda run --no-capture-output -n tokensmith2 python -m src.main add-chapters --partial --chapters $(CHAPTERS) $(ARGS)

run-chat-partial:
	@echo "Running TokenSmith chat mode (partial index) with additional CLI args: $(ARGS)"
	conda run --no-capture-output -n tokensmith2 --no-capture-output python -m src.main chat --partial $(ARGS)
